  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border bg-card p-5 md:p-8">
        <button onClick={() => goToDepartment('Planning and Design')} className="mx-auto block rounded-lg border-2 border-primary bg-primary/5 px-8 py-4 text-sm font-semibold">
            ...
            <div key={unit.name} className="flex flex-col items-center">
              ...
              <div className="mt-4 flex w-full flex-col gap-2">
                {unit.children.map((child) => (
                  <button key={child} onClick={() => goToDepartment(child)} className="rounded-lg border bg-background px-3 py-3 text-center text-xs font-medium text-muted-foreground hover:border-primary hover:text-primary">
                    {child}
                  </button>
                ))}
              </div>
