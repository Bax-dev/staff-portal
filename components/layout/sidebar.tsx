  ...
  return (
    <aside className={`${open ? 'fixed inset-y-0 left-0 z-50 flex' : 'hidden'} w-72 shrink-0 flex-col border-r bg-card lg:flex`}>
      <div className="flex h-20 items-center gap-3 border-b px-7">
        <div className="grid size-9 place-items-center rounded-lg bg-primary text-primary-foreground font-bold">O</div>
        <div>
          <p className="font-semibold tracking-tight">Organo</p>
          <p className="text-xs text-muted-foreground">Staff administration</p>
        </div>
      ...
      <nav className="flex flex-1 flex-col gap-1 p-4">
        ...
        <div className="mt-auto rounded-xl bg-muted/60 p-4">
          ...
          <Link href="/settings" onClick={onClose} className="mt-3 inline-block text-xs font-semibold text-primary">
            Review settings <ChevronRight className="ml-1 inline size-3" />
          </Link>
        </div>
      </nav>
    </aside>
