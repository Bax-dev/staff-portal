...
import { ArchiveStaffDialog, UnarchiveStaffDialog } from '@/components/staff/archive-staff-dialog'
  ...
  return (
    <TourProvider>
      <div className="min-h-screen bg-background text-foreground">
        ...
        {deletingStaff && (
          <DeleteStaffDialog
            person={deletingStaff}
            onCancel={cancelDelete}
            onConfirm={() => {
              confirmDelete().catch((error: unknown) => toastError(error, 'Could not remove this record.'))
            }}
          />
        )}
        {archivingStaff && (
          <ArchiveStaffDialog
            person={archivingStaff}
            onCancel={cancelArchive}
            onConfirm={() => {
              confirmArchive().catch((error: unknown) => toastError(error, 'Could not archive this record.'))
            }}
          />
        )}
        {unarchivingStaff && (
          <UnarchiveStaffDialog
            person={unarchivingStaff}
            onCancel={cancelUnarchive}
            onConfirm={() => {
              confirmUnarchive().catch((error: unknown) => toastError(error, 'Could not restore this record.'))
            }}
          />
        )}
        <TourGuide onNavOpen={setMobileNav} />
      </div>
    </TourProvider>
  )
}
